...
@SlashGroup("репорт")
export class ReportLoadoutCommand extends CommandWithFields {

  static commandType: ReportType = ReportType.Loadout;
  ...
  @Slash({ name: "лодаут", description: "Что-то исчезло из лодаута, а должно быть" })
  async onCommand(interaction: CommandInteraction) {
    return super.onCommand(interaction);
  }

  @ModalComponent({ id: ModalId.ReportLoadout })
